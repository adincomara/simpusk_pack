...
class KhususController extends Controller
{
    public function index(){
        return view('master/data_khusus');
    }
    ...
    public function getData(Request $request){
        ...
        $records = KhususBPJS::select('*');
        $totalData = $records->get()->count();
        if($search != null || $search != ''){
            $records->where(function($q) use ($search){
                $q->orwhere('kdKhusus', 'LIKE', $search.'%');
                $q->orwhere('nmKhusus', 'LIKE', $search.'%');
            });
        }
        ...
        foreach($data as $key => $record){
            ...
            $record->no = $key + $page;
            $action.='<a href="'.route('khusus.ubah',$enc_id).'" class="btn btn-warning btn-xs icon-btn md-btn-flat product-tooltip mb-1" style="min-width:60px" title="Edit"><i class="fa fa-pencil"></i> Ubah</a>&nbsp;';
    ...
    public function ubah($enc_id){
        ...
        if ($dec_id) {
            $khusus  = KhususBPJS::find($dec_id);
            // $induk = Dokter::where('parent',null)->get();
            // $selectedinduk = $poli->parent;

            return view('master_form/khusus_form',compact('enc_id','khusus'));
        } else {
            return view('errors/noaccess');
        }
    }
    public function tambah(){
        return view('master_form/khusus_form');
    }
    public function simpan(Request $req){
        try{
            ...
            $khusus->kdKhusus = $req->kdKhusus;
            $khusus->nmKhusus = $req->nmKhusus;
            $khusus->save();
            DB::commit();
    ...
    public function hapus($enc_id){
        try{
            DB::beginTransaction();
            $dec_id = $this->safe_decode(Crypt::decryptString($enc_id));
            $spesialis = KhususBPJS::find($dec_id);
            $spesialis->delete();
            DB::commit();
            return response()->json([
                'success' => true,
