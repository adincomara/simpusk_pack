...
class SpesialisController extends Controller
{
    public function index(){
        return view('master/data_spesialis');
    }
    ...
    public function getData(Request $request){
        ...
        foreach($data as $key => $record){
            ...
            $record->no = $key + $page;
            $action.='<a href="'.route('spesialis.ubah',$enc_id).'" class="btn btn-warning btn-xs icon-btn md-btn-flat product-tooltip mb-1" style="min-width:60px" title="Edit"><i class="fa fa-pencil"></i> Ubah</a>&nbsp;';
    ...
    public function ubah($enc_id){
        ...
        if ($dec_id) {
            $spesialis  = SpesialisBPJS::find($dec_id);
            // $induk = Dokter::where('parent',null)->get();
            // $selectedinduk = $poli->parent;

            return view('master_form/spesialis_form',compact('enc_id','spesialis'));
        } else {
            return view('errors/noaccess');
        }
    }
    public function tambah(){
        return view('master_form/spesialis_form');
    }
    public function simpan(Request $req){
        try{
            ...
            $spesialis->kdSpesialis = $req->kdSpesialis;
            $spesialis->nmSpesialis = $req->nmSpesialis;
            $spesialis->save();
            DB::commit();
