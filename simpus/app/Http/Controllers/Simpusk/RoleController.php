...
use App\Models\Simpusk\Role;
use App\Models\Simpusk\RoleUser;
use App\Models\Simpusk\Permission;
...
class RoleController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        foreach ($data as $key=> $result)
        {
          ...
          if($request->user()->can('role.user')){
            $action.='<a href="'.route('role.user',$enc_id).'" class="btn btn-success btn-xs icon-btn md-btn-flat product-tooltip mb-1" style="min-width:60px" title="Daftar User" data-original-title="Show"><i class="fa fa-users"></i> Users</a>&nbsp';
          }
          if($request->user()->can('role.ubah')){
            $action.='<a href="'.route('role.ubah',$enc_id).'" class="btn btn-warning btn-xs icon-btn md-btn-flat product-tooltip mb-1" style="min-width:60px" title="Edit"><i class="fa fa-pencil"></i> Ubah</a>&nbsp;';
          }
    ...
    public function form($enc_id = null)
    {
      ...
      return view('keamanan_form/role_form',['dataSet' => $dataSet, 'permissionObj' => $permissionObj, 'checkbox_loop' => $checkbox_loop]);
    }
    ...
    public function formuser($enc_id = null)
    {
        ...
        return view('keamanan_form/role_form_user',['dataSet' => $dataSet,'userObj' => $userObj]);
    }
