...
@section('title', 'Data Pengadaan Obat')
@section('judultable', 'Data Pengadaan Obat')
@section('menu1', 'Master')
@section('menu2', 'Data Pengadaan Obat')
@section('table')
    ...
    <div class="row">
        ...
        <div class="ibox ">
            <div class="ibox-title">
                <h3>Data Pengadaan Obat</h3>
                <div class="ibox-tools">
                    @can('pengadaan_obat.tambah')


                        <a href="{{ route('pengadaan_obat.excel') }}" target="_blank"><button class="btn btn-warning"><i class="fa fa-file-excel"></i> Download</button></a>
                        <a href="{{ route('pengadaan_obat.cetak') }}" target="_blank"><button class="btn btn-success"><i class="fa fa-file-pdf"></i> Cetak PDF</button></a>
                        <a href="{{ route('pengadaan_obat.tambah') }}"><button class="btn btn-primary"><i class="fa fa-plus"></i> Pengadaan Obat </button></a>
                    @endcan
            ...
            <table class="table table-striped table-bordered table-hover dataTables-example coba" id="table_pengadaan_obat" >
            <thead>
   ...
   var tabledata,table_index;
      $(document).ready(function(){
        $.ajaxSetup({
          ...
          table= $('#table_pengadaan_obat').DataTable({
          "processing": true,
          "serverSide": true,
          "stateSave"  : true,
          "deferRender": true,
          ...
          "select" : true,
          "ajax":{
                   "url": "{{ route("pengadaan_obat.getdata") }}",
                   "dataType": "json",
                   "type": "POST",
                   data: function ( d ) {
                     d._token= "{{csrf_token()}}";
       ...
       function deleteData(e,enc_id){
          ...
          if (result.value) {
            $.ajaxSetup({
             ...
             $.ajax({
              type: 'DELETE',
              url: '{{route("pengadaan_obat.hapus",[null])}}/' + enc_id,
